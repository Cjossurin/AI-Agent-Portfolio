"""Dark Comic Image Generator — dedicated Flux agent for the dark_comic style.
...
import fal_client
import requests
from prompt_templates import CHUCKY_DARK_COMIC_IMAGE_PROMPT_TEMPLATE

logger = logging.getLogger(__name__)

FLUX_MODEL_ID = "fal-ai/flux-pro/v1.1"
STYLE_NAME = "dark_comic"

MAX_RETRIES = 3
RETRY_DELAY = 5
API_TIMEOUT = 300

PROMPT_TEMPLATE = CHUCKY_DARK_COMIC_IMAGE_PROMPT_TEMPLATE


class DarkComicImageGenerator:
    """Generates Flux images in the Dark Comic Animation style via text-only prompts."""
    ...
    def _enrich_prompt(self, prompt: str) -> str:
        """Apply the dark comic style prompt template to the scene description."""
    ...
    def generate_single_image(self, prompt: str, out_path: Path) -> str:
        ...
        logger.info("  Text-only generation (dark_comic)")
    ...
    def generate_images(self, storyboard_json: dict, case_id: str) -> list[dict]:
        ...
        logger.info("Dark Comic image generation — text-only")
        results: list[dict] = []
        ...
        for entry in blocks:
            ...
            logger.info("── Block %d (dark_comic) ──", bid)
        ...
        logger.info("Dark comic images complete for case_%s (%d blocks).", case_id, len(results))
        return results
